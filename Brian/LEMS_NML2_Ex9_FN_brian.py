...
'''
    This Brian file has been generated by org.neuroml.export (see https://github.com/NeuroML/org.neuroml.export)
         org.neuroml.export  v1.4.0
         org.neuroml.model   v1.4.0
         jLEMS               v0.9.7.2
'''
from brian import *
...
if show_gui:
    ...
    trace_d1_V = StateMonitor(fnPop1,'V',record=[0]) # V (Type: Line:  scale=1.0 (dimensionless) timeScale=1.0 (dimensionless))
    trace_d1_W = StateMonitor(fnPop1,'W',record=[0]) # W (Type: Line:  scale=1.0 (dimensionless) timeScale=1.0 (dimensionless))

# Saving to file: results/ex9.dat, ref of1
record_of1 = {}
record_of1["V"] = StateMonitor(fnPop1,'V',record=[0]) # V (Type: OutputColumn)
record_of1["W"] = StateMonitor(fnPop1,'W',record=[0]) # W (Type: OutputColumn)

defaultclock.dt = 0.05*second
run(200*second)

# Saving to file: results/ex9.dat, ref of1
all_of1 = np.array( [ record_of1["V"][0] , record_of1["W"][0]  ] )
...
if show_gui:
    ...
    plot_V.plot(trace_d1_V.times/second,trace_d1_V[0], color="#ee40FF", label="V")
    plot_V.legend()
    plot_W = display_d1.add_subplot(111, autoscale_on=True)
    plot_W.plot(trace_d1_W.times/second,trace_d1_W[0], color="#BBA0AA", label="W")
    plot_W.legend()
    plt.show()
